...
public class TileEntityEnchantedFlower extends TileEntity implements ITickable {
	...
	/**
	 * The colors of the beacon segment are changed based on the glas they pass through. I *may* modify this in the future to have the color match the flower color, but that is not a priority now.
	 */
	private void updateSegmentColors() {
		...
		for (int i1 = k + 1; i1 < 256; ++i1) {
			...
			if (!flag) {
				afloat = new float[] { (customTileEntityBeacon$beamsegment.getColors()[0] + afloat[0]) / 2.0F, (customTileEntityBeacon$beamsegment.getColors()[1] + afloat[1]) / 2.0F, (customTileEntityBeacon$beamsegment.getColors()[2] + afloat[2]) / 2.0F };
			}
