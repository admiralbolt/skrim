...
public class EntityFox extends EntityTameable {
	...
	private static final DataParameter<Integer> COLLAR_COLOR = EntityDataManager.<Integer>createKey(EntityFox.class, DataSerializers.VARINT);
	...
	protected void initEntityAI() {
		...
		this.tasks.addTask(3, new EntityFox.AIAvoidEntity(this, EntityFox.class, 24.0F, 1.5D, 1.5D));
	...
	protected SoundEvent getAmbientSound() {
		return this.isAngry() ? SoundEvents.ENTITY_WOLF_GROWL
				: (this.rand.nextInt(3) == 0 ? (this.isTamed() && ((Float) this.dataManager.get(DATA_HEALTH_ID)).floatValue() < 10.0F
						? SoundEvents.ENTITY_WOLF_WHINE : SoundEvents.ENTITY_WOLF_PANT) : SoundEvents.ENTITY_WOLF_AMBIENT);
	}
		...
		if (this.isWet()) {
			...
			if (this.timeWolfIsShaking > 0.4F) {
				...
				for (int j = 0; j < i; ++j) {
					...
					this.worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, this.posX + (double) f1, (double) (f + 0.8F), this.posZ + (double) f2,
	...
	public float getTailRotation() {
		return this.isAngry() ? 1.5393804F
				: (this.isTamed() ? (0.55F - (this.getMaxHealth() - ((Float) this.dataManager.get(DATA_HEALTH_ID)).floatValue()) * 0.02F) * (float) Math.PI
						: ((float) Math.PI / 5F));
	}
	...
	public boolean canMateWith(EntityAnimal otherAnimal) {
		if (otherAnimal == this) {
			...
			EntityFox EntityFox = (EntityFox) otherAnimal;
			return !EntityFox.isTamed() ? false : (EntityFox.isSitting() ? false : this.isInLove() && EntityFox.isInLove());
		}
	}
	...
	public boolean shouldAttackEntity(EntityLivingBase p_142018_1_, EntityLivingBase p_142018_2_) {
		if (!(p_142018_1_ instanceof EntityCreeper) && !(p_142018_1_ instanceof EntityGhast)) {
			if (p_142018_1_ instanceof EntityFox) {
				EntityFox EntityFox = (EntityFox) p_142018_1_;

				if (EntityFox.isTamed() && EntityFox.getOwner() == p_142018_2_) {
					return false;
				}
			}
	...
	class AIAvoidEntity<T extends Entity> extends EntityAIAvoidEntity<T> {
		private final EntityFox field_190856_d;
		...
		public boolean shouldExecute() {
			return super.shouldExecute() && this.closestLivingEntity instanceof EntityLlama
					? !this.field_190856_d.isTamed() && this.func_190854_a((EntityLlama) this.closestLivingEntity) : false;
		}

		private boolean func_190854_a(EntityLlama p_190854_1_) {
			return p_190854_1_.func_190707_dL() >= EntityFox.this.rand.nextInt(5);
		}
