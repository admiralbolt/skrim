...
/**
 * This is an implementation of the ItemAxe. This class was created due to a bug in ItemAxe that results in an "Array out of Bound" exception in the constructor.
 * 
 * Note: You don't need permission from me to use this code. Just use it if your having the same problem I'm having. :)
 * 
 * @author derf6060
 */
public class CustomAxe extends ItemTool implements ItemModelProvider {
	...
	protected boolean checkStrVsBlock(ItemStack stack, IBlockState state) {
		...
		return b || material == Material.WOOD || material == Material.PLANTS || material == Material.VINE || (((state.getBlock().getHarvestTool(state) != null && state.getBlock().getHarvestTool(state).equals("axe")) ? true : false) && state.getBlock().getHarvestLevel(state) <= this.material.getHarvestLevel());
	}
